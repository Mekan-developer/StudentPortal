...
<?php 
    ...
    if(isset($_POST['submit']) && $_POST['search']!=''){
        $name = $_POST['search'];
        $query = "SELECT * FROM vocabulary where tassyklananmy != 'howwa' and english  LIKE '$name%'";
...
$query = "SELECT * FROM vocabulary where tassyklananmy != 'howwa' order by id desc";
$result = mysqli_query($connect,$query);
$id_count=0;
while($row = mysqli_fetch_assoc($result)){
    $id = $row['id']; $id_count++;  
...
header('location:index.php?page=rugsatsyz_sozler');
ob_end_flush();
 } ?>     
 </tbody>
</table>
